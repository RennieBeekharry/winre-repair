...
rem WR_ACTION=START_RESCUEMEAI_DUAL_FORMAT_AUTH_V30
...
set "COMMAND_VERSION=RMAI-2026.08.15-AGENT-START-30"
...
set "JOUT=%WORK%\start30-jget.txt"
set "FAILFILE=%WORK%\START30_FAILURE.txt"
...
set "LAST_SUCCESS=START-30 entered"
set "ERROR_ID=RMAI-START30-UNKNOWN"
...
for %%F in ("%CURL%" "%FINDSTR%" "%CSCRIPT%" "%PING%" "%AGENT%" "%HELPER%") do if not exist %%F (
  set "ERROR_ID=RMAI-START30-DEP-001"
  set "ERROR_STAGE=STARTUP"
...
for %%F in (network.cmd resolve.cmd reporting.cmd safety.cmd agent-core.js ui.cmd) do if not exist "%RUNTIME%\%%F" (
  set "ERROR_ID=RMAI-START30-RUNTIME-001"
  set "ERROR_STAGE=STARTUP"
...
if not defined APIIP (
  set "ERROR_ID=RMAI-START30-ROUTE-001"
...
if not defined WEBIP (
  set "ERROR_ID=RMAI-START30-ROUTE-002"
  set "ERROR_STAGE=NETWORK ROUTE"
...
rem Prove both supported response parsers locally before contacting GitHub.
>"%WORK%\start30-selftest.json" echo {"device_code":"JSON_OK","expires_in":900}
call :GET_VALUE "%WORK%\start30-selftest.json" device_code SELFTEST
if errorlevel 1 (
  set "ERROR_ID=RMAI-START30-PARSER-JSON-001"
...
if /i not "!SELFTEST!"=="JSON_OK" (
  set "ERROR_ID=RMAI-START30-PARSER-JSON-002"
  set "FAIL_REASON=The JSON parser returned the wrong self-test value."
  goto :FATAL
)
>"%WORK%\start30-selftest.form" echo device_code=FORM_OK^&user_code=ABCD-EFGH^&expires_in=900^&interval=5
call :FORM_GET "%WORK%\start30-selftest.form" device_code SELFTEST_FORM
if errorlevel 1 (
  set "ERROR_ID=RMAI-START30-PARSER-FORM-001"
  set "ERROR_STAGE=AUTH PARSER SELF-TEST"
...
if /i not "!SELFTEST_FORM!"=="FORM_OK" (
  set "ERROR_ID=RMAI-START30-PARSER-FORM-002"
...
rem Always request fresh authorization; do not trust the currently saved token.
...
if errorlevel 1 (
  set "ERROR_ID=RMAI-START30-OAUTH-PARSE-002"
...
set /a COUNT+=1
if !COUNT! GTR !MAX! (
  set "ERROR_ID=RMAI-START30-OAUTH-EXPIRED-001"
  set "ERROR_STAGE=DEVICE AUTHORIZATION"
...
if errorlevel 1 (
  set "ERROR_ID=RMAI-START30-WAIT-001"
  set "ERROR_STAGE=DEVICE AUTHORIZATION"
...
set "TOK=%WORK%\start30-token-response.txt"
set "TOKHTTP=%WORK%\start30-token-http.txt"
set "TOKHEAD=%WORK%\start30-token-headers.txt"
for %%F in ("%TOK%" "%TOKHTTP%" "%TOKHEAD%") do if exist %%F del /f /q %%F >nul 2>&1
"%CURL%" %TLS% --silent --show-error --connect-timeout 15 --max-time 120 --resolve "github.com:443:%WEBIP%" -X POST -H "Accept: application/json" -H "Content-Type: application/x-www-form-urlencoded" --data-urlencode "client_id=%CLIENT_ID%" --data-urlencode "device_code=!DEVICE_CODE!" --data-urlencode "grant_type=urn:ietf:params:oauth:grant-type:device_code" -D "%TOKHEAD%" "https://github.com/login/oauth/access_token" -o "%TOK%" -w "%%{http_code}" >"%TOKHTTP%" 2>"%WORK%\start30-token-curl.txt"
set "LAST_CURL=!errorlevel!"
set "LAST_HTTP="
...
if /i "!OAUTH_ERROR!"=="access_denied" (
  set "ERROR_ID=RMAI-START30-OAUTH-DENIED-001"
  set "ERROR_STAGE=DEVICE AUTHORIZATION"
  set "FAIL_REASON=GitHub device authorization was denied."
  goto :FATAL
)
if /i "!OAUTH_ERROR!"=="expired_token" (
  set "ERROR_ID=RMAI-START30-OAUTH-EXPIRED-002"
  set "ERROR_STAGE=DEVICE AUTHORIZATION"
  set "FAIL_REASON=GitHub device authorization code expired."
  goto :FATAL
)
set "POLL_STATE=GitHub response received but approval is not complete yet"
goto :DEVICE_POLL
...
set "LAST_SUCCESS=GitHub device authorization returned a candidate credential parsed as !TOKEN_RESPONSE_FORMAT!"

rem GitHub API is authoritative: validate identity before storing anything.
...
set "OUT=%WORK%\start30-user-test.json"
set "HTTP=%WORK%\start30-user-test-http.txt"
"%CURL%" %TLS% --silent --show-error --connect-timeout 15 --max-time 60 --resolve "api.github.com:443:%APIIP%" -H "Accept: application/vnd.github+json" -H "Authorization: Bearer !CANDIDATE_ACCESS!" -H "X-GitHub-Api-Version: 2022-11-28" "https://api.github.com/user" -o "%OUT%" -w "%%{http_code}" >"%HTTP%" 2>"%WORK%\start30-user-test-curl.txt"
set "LAST_CURL=!errorlevel!"
set "IDENTITY_HTTP="
if exist "%HTTP%" set /p "IDENTITY_HTTP="<"%HTTP%"
set "LAST_HTTP=!IDENTITY_HTTP!"
if not "!LAST_CURL!"=="0" (
  set "ERROR_ID=RMAI-START30-IDENTITY-NET-001"
  set "FAIL_REASON=Candidate token identity validation failed at the HTTPS transport layer."
  goto :FATAL
)
if not "!IDENTITY_HTTP!"=="200" (
  set "ERROR_ID=RMAI-START30-IDENTITY-001"
...
set "OUT=%WORK%\start30-repo-test.json"
set "HTTP=%WORK%\start30-repo-test-http.txt"
"%CURL%" %TLS% --silent --show-error --connect-timeout 15 --max-time 60 --resolve "api.github.com:443:%APIIP%" -H "Accept: application/vnd.github+json" -H "Authorization: Bearer !CANDIDATE_ACCESS!" -H "X-GitHub-Api-Version: 2022-11-28" "https://api.github.com/repositories/%LOG_REPO_ID%" -o "%OUT%" -w "%%{http_code}" >"%HTTP%" 2>"%WORK%\start30-repo-test-curl.txt"
set "LAST_CURL=!errorlevel!"
set "REPO_HTTP="
if exist "%HTTP%" set /p "REPO_HTTP="<"%HTTP%"
set "LAST_HTTP=!REPO_HTTP!"
if not "!LAST_CURL!"=="0" (
  set "ERROR_ID=RMAI-START30-REPO-NET-001"
  set "FAIL_REASON=Private repository validation failed at the HTTPS transport layer."
  goto :FATAL
)
if not "!REPO_HTTP!"=="200" (
  set "ERROR_ID=RMAI-START30-REPO-001"
...
rem Save only the credential that GitHub itself validated.
...
rem Stage the corrected agent-side auth module only after authorization is proven.
...
if errorlevel 1 (
  set "ERROR_ID=RMAI-START30-HANDOFF-MARKER-001"
...
if errorlevel 1 (
  set "ERROR_ID=RMAI-START30-HANDOFF-COPY-001"
...
>>"%RUNTIME%\runtime-sync.cmd" echo rem WR-MODULE: runtime-local-ready 2026.08.15-START-30
...
set "ARC=!errorlevel!"
if "!ARC!"=="0" exit /b 0
set "ERROR_ID=RMAI-START30-AGENT-001"
...
:GET_VALUE
set "%~3="
set "LAST_FORMAT=UNKNOWN"
call :JGET "%~1" "%~2" "%~3"
if not errorlevel 1 (
  set "LAST_FORMAT=JSON"
  set "RESPONSE_FORMAT=JSON"
  exit /b 0
)
call :FORM_GET "%~1" "%~2" "%~3"
if not errorlevel 1 (
  set "LAST_FORMAT=FORM"
...
"%CSCRIPT%" //nologo "%HELPER%" "%~1" "%~2" >"%JOUT%" 2>"%WORK%\start30-jget-error.txt"
...
echo   Response format detected: !DEVICE_FORMAT!
echo.
echo WHAT HAPPENS NEXT
echo ----------------------------------------------------------------------------------------------------
echo   RescueMeAI will detect approval automatically.
echo   The returned credential is tested against GitHub identity and the private recovery repository
