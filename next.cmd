...
rem WR_SUMMARY=Stage and start the authenticated RescueMeAI persistent recovery agent.
rem WR_ACTION=START_RESCUEMEAI_AGENT
rem WR_TARGET=RescueMeAI recovery tooling and private command channel only.
rem WR_CONSEQUENCE=Creates local agent configuration, stages pinned RescueMeAI runtime files, and starts the listener.
rem WR_ROLLBACK=Stop the agent or remove staged RescueMeAI tooling. No Windows repair state is changed by this transition.

set "COMMAND_VERSION=RMAI-2026.08.14-AGENT-START-1"
...
set "SOURCE_REF=d6b8e611ae29e3b5bbc37a86ad9e248a20dd0d8e"
...
call :HEADER INFO "STARTING PERSISTENT RECOVERY AGENT"
call :SECTION "PRIVATE CHANNEL"
call :WRAP "GitHub App pairing is already complete. RescueMeAI will reuse the saved authorization and start the persistent validated command listener."
...
if not exist "%TOKEN%" (
  set "FAIL_RC=90"
  set "FAIL_REASON=The saved RescueMeAI GitHub authorization token is missing. Do not proceed to recovery commands until authorization is restored."
  goto :FAIL
)
...
call :FETCH_PINNED "wr-agent.cmd" "%AGENT%" "AGENT_VERSION=AI-RECOVERY-AGENT-2026.08.14-1105-ET"
...
call :SECTION "WHAT HAPPENS NOW"
call :WRAP "RescueMeAI is starting the authenticated listener. Keep this Command Prompt window open. The listener will check the private command queue automatically."
call :WRAP "The first queued action is a READ-ONLY PING. It does not modify Windows."
call :WRAP "When the agent is online, this window is expected to remain active instead of returning immediately to the command prompt."
echo.
...
if "!AGENT_RC!"=="0" (
  ...
  call :SECTION "AGENT EXITED"
  call :WRAP "The RescueMeAI listener stopped normally. Run C:\wr.cmd again only when you want to restart the listener."
  color 07 >nul 2>&1
  title Command Prompt
  exit /b 0
)
set "FAIL_RC=!AGENT_RC!"
set "FAIL_REASON=The RescueMeAI persistent agent exited unexpectedly. Review the private report before retrying."
goto :FAIL
...
:FAIL
call :HEADER ERROR "PERSISTENT AGENT STARTUP FAILED"
call :SECTION "[FAIL] RESCUEMEAI AGENT DID NOT STAY ONLINE"
call :LINE "Return code : !FAIL_RC!"
call :SECTION "REASON"
call :WRAP "!FAIL_REASON!"
call :SECTION "SAFETY"
call :WRAP "No Windows repair, disk, boot, registry, partition, or filesystem action was performed by this startup transition."
call :SECTION "NEXT"
call :WRAP "RescueMeAI is returning automatically to the Windows Recovery command prompt. Private reporting should be reviewed before retrying."
call :LINE "%UI_BORDER%"
echo.
