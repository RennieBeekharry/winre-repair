...
rem WR_SUMMARY=Load the immutable RescueMeAI Pairing-11 core and continue repository-scoped GitHub App device pairing.
...
set "SOURCE_REF=a7fbf777d22c8e563a976a6872c5b6bd41221f14"
set "CORE=%RUNTIME%\pairing-core.cmd"
set "CORE_TMP=%RUNTIME%\pairing-core.cmd.tmp"
set "CORE_URL=https://api.github.com/repos/%SOURCE_REPO%/contents/lib/pairing-core.cmd?ref=%SOURCE_REF%"
set "APIIP="
set "DOH_STATUS=NOT_RUN"
set "DOH_HTTP=NOT_RUN"
set "DOH_CURL_RC=NOT_RUN"
set "FAIL_RC=90"
set "FAIL_REASON=RescueMeAI could not load the secure pairing core."
set "UI_WIDTH=96"
...
call :WRAP "RescueMeAI is loading the immutable Pairing-11 authentication core. Terms version 2026-08-14 was already accepted on this recovery computer."
...
if not defined APIIP (
  set "FAIL_RC=92"
  set "FAIL_REASON=DNS-over-HTTPS could not provide an api.github.com address for the pinned pairing core."
  goto :FAIL
)
...
if not "!FETCH_RC!"=="0" (
  set "FAIL_RC=90"
  set "FAIL_REASON=The pinned RescueMeAI Pairing-11 core could not be downloaded."
  goto :FAIL
)
if not exist "%CORE_TMP%" (
  set "FAIL_RC=90"
  set "FAIL_REASON=curl returned success but the Pairing-11 core file was not created."
  goto :FAIL
)
for %%Z in ("%CORE_TMP%") do if %%~zZ LSS 256 (
  set "FAIL_RC=96"
  set "FAIL_REASON=The downloaded Pairing-11 core was unexpectedly small."
  goto :FAIL
)
"%FINDSTR%" /i /c:"WR-MODULE: pairing-core 2026.08.14-PAIRING-11" "%CORE_TMP%" >nul 2>&1
if errorlevel 1 (
  set "FAIL_RC=96"
  set "FAIL_REASON=The downloaded Pairing-11 core failed marker validation."
  goto :FAIL
)
move /y "%CORE_TMP%" "%CORE%" >nul 2>&1
if errorlevel 1 (
  set "FAIL_RC=97"
  set "FAIL_REASON=The validated Pairing-11 core could not be staged locally."
  goto :FAIL
)
...
call :SECTION "[FAIL] RESCUEMEAI COULD NOT LOAD PAIRING-11"
