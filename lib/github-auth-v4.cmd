...
rem WR-MODULE: github-auth-v4 2026.08.15-FORM-OAUTH-TLS
rem RescueMeAI GitHub App authorization for WinRE.
rem Uses GitHub OAuth/device-flow default x-www-form-urlencoded responses.
rem No JSON parser dependency.
...
call :FORM_GET "%F%" access_token GA_NEW_ACCESS
if not defined GA_NEW_ACCESS (
  call :FORM_GET "%F%" error GA_ERROR
...
call :FORM_GET "%F%" refresh_token GA_NEW_REFRESH
call :FORM_GET "%F%" expires_in GA_ACCESS_EXPIRES
call :FORM_GET "%F%" refresh_token_expires_in GA_REFRESH_EXPIRES
...
:DEVICE_FLOW
set "D=%GA_WORK%\github-device-v16.txt"
set "DH=%GA_WORK%\github-device-v16-http.txt"
"%GA_CURL%" %GA_TLS% --silent --show-error --connect-timeout 15 --max-time 120 --resolve "%GA_WEBHOST%:443:%GA_WEBIP%" -X POST -H "Content-Type: application/x-www-form-urlencoded" --data-urlencode "client_id=%GA_CLIENTID%" "https://github.com/login/device/code" -o "%D%" -w "%%{http_code}" >"%DH%" 2>"%GA_WORK%\github-device-v16-curl.txt"
set "GA_LAST_CURL=!errorlevel!"
set "GA_LAST_HTTP="
...
call :FORM_GET "%D%" device_code GA_DEVICE_CODE
call :FORM_GET "%D%" user_code GA_USER_CODE
call :FORM_GET "%D%" expires_in GA_EXPIRES
call :FORM_GET "%D%" interval GA_INTERVAL
if not defined GA_DEVICE_CODE (
  call :FAIL 96 "GitHub returned HTTP 200 but no form-encoded device_code was found."
  exit /b 96
)
if not defined GA_USER_CODE (
  call :FAIL 96 "GitHub returned a device code but no user_code was found."
  exit /b 96
)
...
set "T=%GA_WORK%\github-token-v16.txt"
set "TH=%GA_WORK%\github-token-v16-http.txt"
"%GA_CURL%" %GA_TLS% --silent --show-error --connect-timeout 15 --max-time 120 --resolve "%GA_WEBHOST%:443:%GA_WEBIP%" -X POST -H "Content-Type: application/x-www-form-urlencoded" --data-urlencode "client_id=%GA_CLIENTID%" --data-urlencode "device_code=!GA_DEVICE_CODE!" --data-urlencode "grant_type=urn:ietf:params:oauth:grant-type:device_code" --data-urlencode "repository_id=%GA_REPOID%" "https://github.com/login/oauth/access_token" -o "%T%" -w "%%{http_code}" >"%TH%" 2>"%GA_WORK%\github-token-v16-curl.txt"
set "GA_LAST_CURL=!errorlevel!"
set "GA_LAST_HTTP="
if exist "%TH%" set /p "GA_LAST_HTTP="<"%TH%"
if not "!GA_LAST_CURL!"=="0" goto :DEVICE_POLL
if not "!GA_LAST_HTTP!"=="200" goto :DEVICE_POLL
call :FORM_GET "%T%" access_token GA_NEW_ACCESS
if defined GA_NEW_ACCESS (
  call :FORM_GET "%T%" refresh_token GA_NEW_REFRESH
  call :FORM_GET "%T%" expires_in GA_ACCESS_EXPIRES
  call :FORM_GET "%T%" refresh_token_expires_in GA_REFRESH_EXPIRES
  call :STORE_TOKEN "!GA_NEW_ACCESS!" "!GA_NEW_REFRESH!" DEVICE
  set "GA_NEW_ACCESS="
  set "GA_NEW_REFRESH="
  exit /b 0
)
call :FORM_GET "%T%" error GA_ERROR
...
set "F=%GA_WORK%\github-token-test-v4.json"
set "H=%GA_WORK%\github-token-test-v4-http.txt"
"%GA_CURL%" %GA_TLS% --silent --show-error --connect-timeout 15 --max-time 60 --resolve "%GA_APIHOST%:443:%GA_APIIP%" -H "Accept: application/vnd.github+json" -H "Authorization: Bearer !GA_ACCESS!" -H "X-GitHub-Api-Version: 2022-11-28" "https://api.github.com/repositories/%GA_REPOID%" -o "%F%" -w "%%{http_code}" >"%H%" 2>"%GA_WORK%\github-token-test-v4-curl.txt"
set "GA_LAST_CURL=!errorlevel!"
set "GA_LAST_HTTP="
if exist "%H%" set /p "GA_LAST_HTTP="<"%H%"
set "GA_ACCESS="
...
>"%GA_META%" echo auth_type=github_app_user_access_token
>>"%GA_META%" echo source=!S!
>>"%GA_META%" echo app_id=%GA_APPID%
>>"%GA_META%" echo client_id=%GA_CLIENTID%
>>"%GA_META%" echo repository=%GA_LOGREPO%
...
"%GA_CURL%" %GA_TLS% --silent --show-error --connect-timeout 15 --max-time 120 --resolve "%GA_APIHOST%:443:%GA_APIIP%" -X PUT -H "Accept: application/vnd.github+json" -H "Authorization: Bearer !GA_ACCESS!" -H "X-GitHub-Api-Version: 2022-11-28" -H "Content-Type: application/json" --data-binary "@%REQ%" -o "%RESP%" -w "%%{http_code}" "https://api.github.com/repos/%GA_LOGREPO%/contents/!P!" >"%HTTP%" 2>"%GA_WORK%\github-upload-v4-curl.txt"
set "GA_LAST_CURL=!errorlevel!"
set "GA_LAST_HTTP="
...
for %%P in (!FG_SCAN!) do (
  set "FG_PAIR=%%~P"
  for /f "tokens=1,* delims==" %%A in ("!FG_PAIR!") do (
    if /i "%%A"=="%~2" set "%~3=%%B"
  )
)
set "FG_LINE="
