...
rem WR-MODULE: github-auth 2026.08.15-V5-ATOMIC-CREDENTIALS
...
rem refreshes, and uploads with safe structured diagnostics. Credential writes are atomic and verified.
...
set "GA_NEW_ACCESS="
set "GA_NEW_REFRESH="
set "GA_LAST_SUCCESS=refresh endpoint returned and atomically stored a replacement credential"
exit /b 0
...
set "GA_TOKEN_NEW=%GA_AUTHDIR%\github-logs.token.new"
set "GA_REFRESH_NEW=%GA_AUTHDIR%\github-refresh.token.new"
for %%F in ("%GA_TOKEN_NEW%" "%GA_REFRESH_NEW%") do (
  if exist %%F attrib -h -s -r %%F >nul 2>&1
  if exist %%F del /f /q /a %%F >nul 2>&1
)
>"%GA_TOKEN_NEW%" echo(!GA_STORE_ACCESS!
if errorlevel 1 exit /b 1
set "GA_VERIFY="
set /p "GA_VERIFY="<"%GA_TOKEN_NEW%"
if not defined GA_VERIFY exit /b 1
if not "!GA_VERIFY!"=="!GA_STORE_ACCESS!" exit /b 1
if defined GA_STORE_REFRESH (
  >"%GA_REFRESH_NEW%" echo(!GA_STORE_REFRESH!
  if errorlevel 1 exit /b 1
)
if exist "%GA_TOKEN%" attrib -h -s -r "%GA_TOKEN%" >nul 2>&1
if exist "%GA_TOKEN%" del /f /q /a "%GA_TOKEN%" >nul 2>&1
if exist "%GA_TOKEN%" exit /b 1
move /y "%GA_TOKEN_NEW%" "%GA_TOKEN%" >nul 2>&1
if errorlevel 1 exit /b 1
if not exist "%GA_TOKEN%" exit /b 1
attrib +h +s "%GA_TOKEN%" >nul 2>&1
if exist "%GA_REFRESH%" attrib -h -s -r "%GA_REFRESH%" >nul 2>&1
if exist "%GA_REFRESH%" del /f /q /a "%GA_REFRESH%" >nul 2>&1
if defined GA_STORE_REFRESH (
  move /y "%GA_REFRESH_NEW%" "%GA_REFRESH%" >nul 2>&1
  if errorlevel 1 exit /b 1
  if not exist "%GA_REFRESH%" exit /b 1
  attrib +h +s "%GA_REFRESH%" >nul 2>&1
)
set "GA_VERIFY="
set /p "GA_VERIFY="<"%GA_TOKEN%"
if not defined GA_VERIFY exit /b 1
if not "!GA_VERIFY!"=="!GA_STORE_ACCESS!" exit /b 1
exit /b 0
...
>>"%GA_RESULT%" echo component=github-auth-v5-agent
