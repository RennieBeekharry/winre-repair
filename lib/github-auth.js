// WR-MODULE: github-auth-js 2026.08.14-0948-ET
(function () {
    var a = WScript.Arguments;
    if (a.length < 9) WScript.Quit(91);

    var mode = String(a(0)).toLowerCase();
    var curl = a(1), nslookup = a(2), dns = a(3), work = a(4);
    var logrepo = a(5), tokenfile = a(6), report = a(7), details = a(8);

    var fso = new ActiveXObject("Scripting.FileSystemObject");
    var shell = new ActiveXObject("WScript.Shell");
    var resultFile = work + "\\GITHUB_RESULT.txt";
    var clientId = "178c6fc778ccc68e1d6a";
    ...
    function resolveHost(host) {
        try {
            var e = shell.Exec(q(nslookup) + " " + host + " " + dns);
            var out = "";
    ...
    function httpCode(statusFile) {
        var s = readText(statusFile).replace(/\s+/g, "");
        return s || "000";
    }
    ...
    function base64Utf8(s) {
        ...
        for (i = 0; i < b.length; i += 3) {
            a1 = b[i];
            a2 = (i + 1 < b.length) ? b[i + 1] : -1;
            a3 = (i + 2 < b.length) ? b[i + 2] : -1;
    ...
    function uploadWithToken(token, label) {
        var text = "PRIVATE WINRE RECOVERY RUN REPORT\r\n==================================\r\n";
        text += readText(report);
        if (details && fso.FileExists(details)) {
            text += "\r\n--- RUN_DETAILS ---\r\n" + readText(details);
        }

        var safeLabel = String(label || "run").replace(/[^A-Za-z0-9._-]/g, "_");
        var stamp = (new Date()).getTime();
        var path = "reports/inbox/" + safeLabel + "-" + stamp + ".txt";
        var req = work + "\\github-upload-request.json";
        var resp = work + "\\github-upload-response.json";
        var stat = work + "\\github-upload-status.txt";
        var payload = '{"message":"WinRE recovery report","content":"' + base64Utf8(text) + '"}';
        writeText(req, payload);

        var host = "api.github.com";
        var url = "https://" + host + "/repos/" + logrepo + "/contents/" + path;
        var args = "-X PUT -H " + q("Accept: application/vnd.github+json") +
        ...
        if (http == "201" || http == "200") {
            writeResult("PASS", "Private report uploaded.", http, path);
            return 0;
        }
        ...
        else if (http == "404") reason = "Authorized account cannot access the private log repository.";
    ...
    function authorize() {
        ...
        var host = "github.com";
        var deviceResp = work + "\\github-device.json";
        var deviceStat = work + "\\github-device-status.txt";
        var deviceArgs = "-X POST -H " + q("Accept: application/json") +
                         " -H " + q("Content-Type: application/x-www-form-urlencoded") +
                         " --data " + q("client_id=" + clientId + "&scope=repo") +
                         " " + q("https://github.com/login/device/code");
        runCurl(host, deviceArgs, deviceResp, deviceStat);
        var dh = httpCode(deviceStat), d = parseJson(deviceResp);
        if (dh != "200" || !d || !d.device_code || !d.user_code) {
            writeResult("FAIL", "Could not start GitHub device authorization.", dh, "");
            return 90;
        }
        ...
        WScript.Echo("");
        WScript.Echo("The PC will wait here automatically after you approve it.");
        WScript.Echo("================================================================");

        var interval = Number(d.interval || 5);
        var deadline = (new Date()).getTime() + (Number(d.expires_in || 900) * 1000);
        var tokenResp = work + "\\github-token.json";
        var tokenStat = work + "\\github-token-status.txt";

        while ((new Date()).getTime() < deadline) {
            WScript.Sleep(interval * 1000);
            var tokenArgs = "-X POST -H " + q("Accept: application/json") +
                            " -H " + q("Content-Type: application/x-www-form-urlencoded") +
                            " --data " + q("client_id=" + clientId +
            ...
            if (t.error == "access_denied") {
                writeResult("FAIL", "GitHub device authorization was denied.", th, "");
                return 90;
            }
            if (t.error == "expired_token") {
                writeResult("FAIL", "GitHub one-time device code expired; rerun C:\\wr.cmd for a new code.", th, "");
                return 90;
            }
            if (t.error) {
                writeResult("FAIL", "GitHub device authorization error: " + t.error, th, "");
                return 90;
            }
        }

        writeResult("FAIL", "GitHub one-time device code expired; rerun C:\\wr.cmd for a new code.", "TIMEOUT", "");
        return 90;
    }
    ...
    if (mode == "upload") {
        if (!fso.FileExists(tokenfile)) {
            writeResult("FAIL", "Private reporting is not authorized yet.", "LOCAL", "");
            WScript.Quit(90);
        }
        var tok = readText(tokenfile).replace(/\s+/g, "");
        if (!tok) {
            writeResult("FAIL", "Saved authorization file is empty.", "LOCAL", "");
            WScript.Quit(90);
        }
        WScript.Quit(uploadWithToken(tok, "run"));
    }

    writeResult("FAIL", "Unknown GitHub authorization module mode.", "LOCAL", "");
    WScript.Quit(91);
})();
