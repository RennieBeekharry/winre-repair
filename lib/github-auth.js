// WR-MODULE: github-auth-js 2026.08.14-1148-ET
(function () {
    ...
    var curlErrorFile = work + "\\GITHUB_CURL_ERROR.txt";
    var apiVersion = "2022-11-28";

    function writeResult(status, reason, http, path, curlRc) {
        var f = fso.CreateTextFile(resultFile, true);
        f.WriteLine("status=" + status);
        f.WriteLine("reason=" + reason);
        f.WriteLine("http=" + (http || ""));
        f.WriteLine("curl_return_code=" + (curlRc === undefined ? "" : curlRc));
    ...
    function validIpv4(s) {
        ...
        for (i = 0; i < 4; i++) { if (!/^\d{1,3}$/.test(p[i])) return false; n = Number(p[i]); if (n < 0 || n > 255) return false; }
        return true;
    }
    function resolveHost(host) {
        try {
            var cache = "";
            if (host === "api.github.com") cache = work + "\\github-api-ip.txt";
            if (host === "github.com") cache = work + "\\github-web-ip.txt";
            if (cache && fso.FileExists(cache)) {
                var cached = readText(cache).replace(/\s+/g, "");
                if (validIpv4(cached)) return cached;
            }
    ...
    function runCurl(host, args, bodyFile, statusFile) {
        ...
        if (statusFile) cmd += " -w " + q("%{http_code}");
        try {
            var e = shell.Exec(cmd), stdout = "", stderr = "";
            ...
            if (statusFile) writeText(statusFile, String(stdout).replace(/\s+/g, ""));
            if (stderr) writeText(curlErrorFile, stderr);
            else if (fso.FileExists(curlErrorFile)) { try { fso.DeleteFile(curlErrorFile, true); } catch (ignore0) {} }
            return e.ExitCode;
        } catch (ex) {
            if (statusFile) writeText(statusFile, "000");
            writeText(curlErrorFile, "WScript.Shell.Exec failed: " + ex.message);
            return 91;
        }
    }
    ...
    function uploadWithToken(token, label) {
        ...
        var crc = runCurl(host, args, resp, stat);
        var http = httpCode(stat);
        if (crc === 0 && (http == "201" || http == "200")) { writeResult("PASS", "Private report uploaded.", http, path, crc); return 0; }
        var reason = "GitHub upload failed.";
        if (crc !== 0 && http == "000") reason = "No GitHub API response; curl returned " + crc + ". Network, DNS, or TLS may have failed.";
        else if (http == "401") reason = "Saved authorization is invalid, expired, or revoked.";
        ...
        writeResult("FAIL", reason, http, "", crc); return 90;
    }
    function authorize() {
        if (!/^[A-Za-z0-9]+$/.test(String(clientId))) { writeResult("FAIL", "OAuth client ID is not configured.", "LOCAL", "", 91); return 91; }
        ...
        var dcrc = runCurl(host, deviceArgs, deviceResp, deviceStat);
        var dh = httpCode(deviceStat), d = parseJson(deviceResp);
        if (dcrc !== 0 || dh != "200" || !d || !d.device_code || !d.user_code) {
            writeResult("FAIL", "Could not start GitHub device authorization.", dh, "", dcrc); return 90;
        }
        WScript.Echo("================================================================");
        ...
        while ((new Date()).getTime() < deadline) {
            ...
            var tcrc = runCurl(host, tokenArgs, tokenResp, tokenStat);
            var th = httpCode(tokenStat), t = parseJson(tokenResp);
            if (tcrc !== 0 || th != "200" || !t) continue;
            if (t.access_token) {
            ...
            if (t.error == "access_denied") { writeResult("FAIL", "GitHub device authorization was denied.", th, "", tcrc); return 90; }
            if (t.error == "expired_token") { writeResult("FAIL", "GitHub one-time device code expired; start authorization again.", th, "", tcrc); return 90; }
            if (t.error) { writeResult("FAIL", "GitHub device authorization error: " + t.error, th, "", tcrc); return 90; }
        }
        writeResult("FAIL", "GitHub one-time device code expired; start authorization again.", "TIMEOUT", "", 0); return 90;
    }
    if (mode == "authorize") WScript.Quit(authorize());
    if (mode == "upload") {
        if (!fso.FileExists(tokenfile)) { writeResult("FAIL", "Private reporting is not authorized yet.", "LOCAL", "", 90); WScript.Quit(90); }
        var tok = readText(tokenfile).replace(/\s+/g, "");
        if (!tok) { writeResult("FAIL", "Saved authorization file is empty.", "LOCAL", "", 90); WScript.Quit(90); }
        WScript.Quit(uploadWithToken(tok, "run"));
    }
    writeResult("FAIL", "Unknown GitHub authorization module mode.", "LOCAL", "", 91); WScript.Quit(91);
})();
