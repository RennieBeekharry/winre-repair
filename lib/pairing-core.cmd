...
rem WR-MODULE: pairing-core 2026.08.14-PAIRING-11
rem Repository-scoped GitHub App device pairing for WinRE.
rem Uses GitHub's default form-encoded device-flow responses instead of JSON.

set "COMMAND_VERSION=RMAI-2026.08.14-SECURE-PAIRING-11"
...
call :UI_SETUP
call :WRITE_REPORT RUNNING 0 "RescueMeAI Pairing-11 started."
...
rem -------------------------------------------------------------------------
rem TERMS - Pairing-11 is a continuation transition. It requires the already
rem recorded acceptance and never silently recreates or weakens that record.
rem -------------------------------------------------------------------------
...
rem -------------------------------------------------------------------------
rem RESOLVE GITHUB.COM WITH DNS-OVER-HTTPS
rem -------------------------------------------------------------------------
...
rem -------------------------------------------------------------------------
rem REQUEST DEVICE CODE USING GITHUB'S DEFAULT FORM-ENCODED RESPONSE
rem -------------------------------------------------------------------------
...
set "POLL_INTERVAL="
set "DEVICE_ERROR="
call :FORM_VALUE "%WORK%\github-device.form" "error" DEVICE_ERROR >nul 2>&1
...
call :FORM_VALUE "%WORK%\github-device.form" "device_code" DEVICE_CODE
if errorlevel 1 (
  set "AUTH_STATUS=DEVICE_CODE_PARSE_FAILED"
  set "FAIL_RC=96"
  set "COMPONENT_RC=96"
  set "FAIL_REASON=GitHub returned HTTP 200 but RescueMeAI could not parse device_code from the form response."
  goto :FAIL
)
call :FORM_VALUE "%WORK%\github-device.form" "user_code" USER_CODE
if errorlevel 1 (
...
call :FORM_VALUE "%WORK%\github-device.form" "expires_in" EXPIRES_IN >nul 2>&1
if not defined EXPIRES_IN set "EXPIRES_IN=900"
call :FORM_VALUE "%WORK%\github-device.form" "interval" POLL_INTERVAL >nul 2>&1
...
for %%W in (!UI_WRAP_TEXT!) do (
  if not defined UI_WRAP_LINE (
    ...
    if !UI_WRAP_LEN! GTR %UI_TEXT_WIDTH% (
      ...
      set "UI_WRAP_LINE=!UI_WRAP_CANDIDATE!!UI_WRAP_CAND!"
...
:REQUEST_DEVICE_CODE
set "DEVICE_FORM=%WORK%\github-device.form"
set "DEVICE_HTTP_FILE=%WORK%\github-device-http.txt"
set "CURLERR=%WORK%\GITHUB_CURL_ERROR.txt"
if exist "%DEVICE_FORM%" del /f /q "%DEVICE_FORM%" >nul 2>&1
if exist "%DEVICE_HTTP_FILE%" del /f /q "%DEVICE_HTTP_FILE%" >nul 2>&1
"%CURL%" --ssl-no-revoke --silent --show-error --connect-timeout 15 --max-time 90 --resolve "%WEBHOST%:443:%WEBIP%" -X POST -H "Content-Type: application/x-www-form-urlencoded" --data "client_id=%GITHUB_APP_CLIENT_ID%" "https://github.com/login/device/code" -o "%DEVICE_FORM%" -w "%%{http_code}" >"%DEVICE_HTTP_FILE%" 2>"%CURLERR%"
...
:POLL_TOKEN
set "TOKEN_FORM=%WORK%\github-token.form"
set "TOKEN_HTTP_FILE=%WORK%\github-token-http.txt"
if exist "%TOKEN_FORM%" del /f /q "%TOKEN_FORM%" >nul 2>&1
if exist "%TOKEN_HTTP_FILE%" del /f /q "%TOKEN_HTTP_FILE%" >nul 2>&1
"%CURL%" --ssl-no-revoke --silent --show-error --connect-timeout 15 --max-time 90 --resolve "%WEBHOST%:443:%WEBIP%" -X POST -H "Content-Type: application/x-www-form-urlencoded" --data "client_id=%GITHUB_APP_CLIENT_ID%&device_code=!DEVICE_CODE!&grant_type=urn:ietf:params:oauth:grant-type:device_code&repository_id=%GITHUB_REPOSITORY_ID%" "https://github.com/login/oauth/access_token" -o "%TOKEN_FORM%" -w "%%{http_code}" >"%TOKEN_HTTP_FILE%" 2>"%WORK%\GITHUB_CURL_ERROR.txt"
set "AUTH_CURL_RC=!errorlevel!"
set "AUTH_HTTP="
...
set "ACCESS_TOKEN="
set "TOKEN_ERROR="
call :FORM_VALUE "%TOKEN_FORM%" "access_token" ACCESS_TOKEN >nul 2>&1
if defined ACCESS_TOKEN (
  set "REFRESH_TOKEN="
  set "TOKEN_EXPIRES_IN="
  set "REFRESH_EXPIRES_IN="
  call :FORM_VALUE "%TOKEN_FORM%" "refresh_token" REFRESH_TOKEN >nul 2>&1
  call :FORM_VALUE "%TOKEN_FORM%" "expires_in" TOKEN_EXPIRES_IN >nul 2>&1
  call :FORM_VALUE "%TOKEN_FORM%" "refresh_token_expires_in" REFRESH_EXPIRES_IN >nul 2>&1
  set "AUTH_STATUS=AUTHORIZED"
  exit /b 0
)
call :FORM_VALUE "%TOKEN_FORM%" "error" TOKEN_ERROR >nul 2>&1
...
set "FAIL_RC=96"
set "COMPONENT_RC=96"
set "FAIL_REASON=GitHub returned an unexpected form-encoded device-flow response."
exit /b 96
...
>"%JSONFILE%" echo {"message":"RescueMeAI Pairing-11 report","content":"!B64!"}
